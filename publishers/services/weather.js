...
const getWeather = city => {
    return new Promise((res, rej) => {
        let cityWeather = unirest.get(`http://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${key}&lang=${language}`);
        res(cityWeather)
    }).then(res => {
        return [
            (res.body.main.temp - 273), // la temperatura la si ottiene in Kelvin
